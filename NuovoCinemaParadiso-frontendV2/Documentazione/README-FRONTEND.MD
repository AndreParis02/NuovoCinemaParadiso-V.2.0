...
export class AuthService {
  ...
  /* ottieniRuoloUtente() versione precedente
  ottieniRuoloUtente(): RuoliUtente | null{
    const ruolo = localStorage.getItem('ruolo');
  ...
  ottieniRuoloUtente(): RuoliUtente | null {
  const raw = localStorage.getItem(this.storagekey); //legge i dati dal local storage
    ...
    try {
      const utente = JSON.parse(raw) as SessioneUtente; //ottiene i dati utente dal json letto dal local storage
      ...
      if (
        ...
        return utente.ruolo;  //restituisce ruolo utente se valido
      }
