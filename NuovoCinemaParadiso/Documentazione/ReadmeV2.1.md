...
- Utente: Simeone
- Data: 03/06/2026
- Descrizione: modifica per associargli un utente
```c#
using System.ComponentModel.DataAnnotations;
using System.ComponentModel.DataAnnotations.Schema;
...
## Movie.cs ( versione 1.1 )

Utente: Greg
Data: 22/05/2026
Descrizione: Aggiunto il campo IsDeleted per considerarlo come eliminato e mantenerlo all'interno del db discriminandolo
...
public class Movie
{
    ...
    // Proprietà che indica il fatto che sia stato eliminato, ma viene lasciato
    // all'interno del db per avere uno storico e non perdere informazioni
...
Aggiunto parametro booleano per indicare l'eliminazione dell'oggetto preservandone i dati
...
public class DtoMovie
{
    ...
    public bool IsDeleted {get;set;} = false;            // Se è da considerarsi eliminato
...
Aggiunto parametro per indicarne l'eliminazione e quindi preservare i dati
...
## AuthController.cs 1.1

Utente: Alessandro Gregorio
Data: 09/06/2026
Descrizione : Gestiti messaggi di errori nell'eliminazione di un utente
(il risultato dell'eliminazione dell'utente non risulta mai nullo, di conseguenza non veniva restituito il messaggio corretto)
...
public class AuthController : ControllerBase
{
    ...
    public async Task<IActionResult> Elimina()
    {
        ...
        // sostituito il tipo di dato da var a IdentiyResult
        IdentityResult risultato = await _authService.EliminaAsync(utenteId);

        // dato che l'oggetto non ritornava nullo sostituito == null con .Succeeded
...
## BigliettoController.cs V1.1

Andrea Bruno 25-05.2026
Cancellazione metodo modifica perchè non utilizzato
...
- Utente: Simeone
- Data: 03/06/2026
- Descrizione: modifica per associargli un utente ed ottenere le sue giftcard
```c#
using Microsoft.AspNetCore.Authorization;
using Microsoft.AspNetCore.Mvc;
using System.Security.Claims;
...
public class GiftCardController : ControllerBase
{
    ...
    {
        // Il backend estrae l'ID utente direttamente dal token di sicurezza in modo automatico
        string? utenteId = User.FindFirstValue(ClaimTypes.NameIdentifier);
        if (utenteId == null)
            return Unauthorized("Utente non autenticato.");

        // ATTENZIONE: Qui dovrai assicurarti che nel GiftCardService esista un metodo 
        // che fa la query sul database filtrando per utenteId
...
## MovieController.cs (versione 1.3)

Utente: Greg
Data: 22/05/2026
Descrizione: Aggiunto endpoint per il service OttieniTuttoStorico
[HttpGet("storico")] // GET api/Movie/storico
...
- Data: 09/06/2026
- Descrizione: Gestione errori con try catch in Modifica
```c#
using Microsoft.AspNetCore.Authorization;
using Microsoft.AspNetCore.Mvc;
using System.Security.Claims;
...
public class ProiezioneController : ControllerBase
{
    ...
    public async Task<IActionResult> Modifica(string id, [FromBody] DtoCreazioneProiezione dto)
    {
        ...
        catch(NotFoundException ex) //gestione eccezione custom lanciata dal service. Ritorna not found
        {
            await _logAzioniService.SalvataggioLogAzioneAsync(utenteId, "Modifica Proiezione", false);
            return NotFound(new { messaggio = ex.Message });
        }
...
public class SalaController : ControllerBase
{
    ...
    public async Task<IActionResult> OttieniTutti()
    {
        ...
        if (utenteId == null)
            return Unauthorized("Utente non autenticato.");
        List<DtoSala> sale = await _salaService.OttieniTuttoAsync();
        //Creata lista delle sali da ritornare
        List<DtoSala> saleFiltrate = new List<DtoSala>();
        //Per ogni sala che non è stata eliminata l'aggiungo alla lista
        ...
        //Ritorno tutte le sale non eliminate
    ...
    //cambiato l'endpoint per l'eliminazione basandosi su un parametro booleano
    [HttpPut("elimina/{id}")] 
    [Authorize(Roles = Ruoli.Operatore)]
    public async Task<IActionResult> Elimina(string id)
    {
...
Rimossa Exception nella modifica
...
public class SalaController : ControllerBase
{
    ...
    public async Task<IActionResult> Modifica(string id, [FromBody] DtoCreazioneSala dto)
    {
        ...
        // RIMOSSA ECEZZIONE DEL NOME DUPLICATO
        catch (ItemNotFoundException ex)
        {
            await _logAzioniService.SalvataggioLogAzioneAsync(
                utenteId,
...
## UtenteController.cs V 1.1

Utente: Lorenzo Laviosa
Data: 27/05/2026
Descrizione: Viene aggiunto nel metodo di ricarica anche la creazione mancante nella prima versione, bloccante per lo svolgimento del front-end. Nel metodo di riscatto
sistemazione valori di ritorno.
...
public class UtenteController : ControllerBase
{
    ...
    public async Task<IActionResult> RiscattaGiftCard([FromBody] DtoCodiceRiscatto dto)
    {
        // 1. Devi recuperare chi sta facendo la richiesta!
        string? utenteId = User.FindFirstValue(ClaimTypes.NameIdentifier);
        if (utenteId == null)
            return Unauthorized("Utente non autenticato.");
        ...
        // 2. Passa ANCHE l'utenteId al service, altrimenti non saprà a chi dare la card
...
Utente: Marco Strazzeri
Data: 22/05/2026
Descrizione : adesso possono acquistare i biglietti anche gli utenti non abbonati
...
## BigliettoService.cs V1.4

Francesco Lorenzi
22/05/2026

modificacato il controllo mettendo il prezzo finale in variabile
```c#
using Microsoft.EntityFrameworkCore;
using NuovoCinemaParadiso.Data;
using NuovoCinemaParadiso.Dtos;
...
## BigliettoService.cs V1.5

Andrea Bruno 25-05-2026
Cancellazione metodo modifica poichè non utilizzato
...
## GestoreService.cs V1.2

Andrea Bruno 25-05-2026
Mpodifica OttieniTuttiBigliettiAsync
...
public class GestoreService
{
    ...
    public async Task<List<DtoBiglietto>> OttieniTuttiBigliettiAsync()
    {
        ...
        // NOTA: qui vengono caricati solo i dati della tabella Biglietti,
        // le entità correlate verranno caricate manualmente nel ciclo.
        List<Biglietto> biglietti = await _contesto.Biglietti.ToListAsync();

        // Lista che conterrà i DTO finali da restituire al client.
        List<DtoBiglietto> risultato = new List<DtoBiglietto>();

        // Cicla ogni biglietto trovato nel database.
        foreach (var bigliettoCorrente in biglietti)
        {
            // Carica la Proiezione collegata al biglietto.
            // Se non esiste, lancia un'eccezione.
            var proiezione = await _contesto.Proiezioni.FindAsync(bigliettoCorrente.ProiezioneId)
                ?? throw new Exception("Proiezione non trovata");

            // Carica il Movie collegato alla Proiezione.
            var movie = await _contesto.Movies.FindAsync(proiezione.MovieId)
                ?? throw new Exception("Movie non trovato");

            // Carica la Sala collegata alla Proiezione.
            var sala = await _contesto.Sale.FindAsync(proiezione.SalaId)
                ?? throw new Exception("Sala non trovata");

            // Carica la TipologiaSala collegata alla Sala.
            var tipologiaSala = await _contesto.TipologieSala.FindAsync(sala.TipologiaSalaId)
                ?? throw new Exception("TipologiaSala non trovata");

            // Carica il Turno collegato alla Proiezione.
            var turno = await _contesto.Turni.FindAsync(proiezione.TurnoId)
                ?? throw new Exception("Turno non trovato");

            // Mappa i dati raccolti nel DTO da restituire.
            risultato.Add(new DtoBiglietto
            {
                ...
                // Dati provenienti dalle entità correlate
                NomeSala = sala.Nome,
                TitoloMovie = movie.Titolo,
                NomeTipologiaSala = tipologiaSala.Nome,
                OraInizio = turno.OraInizio,
        ...
        // Restituisce la lista completa dei biglietti mappati.
        return risultato;
    }
...
public class GiftCardService
{
    ...
    {
        // Filtriamo le giftcard direttamente nel database usando l'ID utente
        // ATTENZIONE: Assumo che nel tuo modello GiftCard (in C#) la proprietà 
        // che collega la carta all'utente si chiami "UtenteId". 
        // Se si chiama in un altro modo (es. ProprietarioId), modificala qui sotto!
...
<details><summary>Versione 1.3</summary>

- Utente: Alessandro Gregorio
- Data: 09/06/2026
- Descrizione : Aggiunto controllo sull'ottenimento delle giftcard così mostrare all'utente solo quelle riscattabili
...
public class GiftCardService
{
    ...
    public async Task<List<DtoGiftCard>> OttieniPerUtenteAsync(string utenteId)
    {
        ...
        foreach (var giftCardCorrente in giftCards)
        {
        // aggiunto controllo giftcard se è riscattabile o meno
...
## MovieService.cs (Versione 1.3)

Utente: Greg
Data: 22/05/2026
Descrizione: Aggiunto OttieniTuttoStorico per mostrare anche i movie con IsDeleted = true + OttieniTutto solo quelli con IsDeleted = false + Disabilitazione delle proiezioni con determinate codnizioni dopo aver eliminato un film
...
public class MovieService
{
    ...
    /// <summary>
    /// Recupera tutti i film presenti nel sistema con informazioni sul genere non considerando quelli IsDeleted.
    /// </summary>
    /// <returns>Lista di DTO dei film.</returns>
    public async Task<List<DtoMovie>> OttieniTutto()
    {
    ...
    /// <summary>
    /// Recupera tutti i film presenti nel sistema considerando anche quelli IsDeleted.
    /// </summary>
    /// <returns>Lista di DTO dei film.</returns>
    /// 
    ...
    public async Task<DtoMovie?> OttieniTramiteIdAsync(string id)
    {
        ...
        DtoMovie risultato = new DtoMovie
        {
            ...
            GenereId = movie.GenereId,
            Genere = genereMovie?.Genere ?? "",
            // AGGINTO SOLO NEL TRAMITE SOLO OttieniTramiteIdAsync E NON GENERE O ALTRO
    ...
    // corretta la gestione degli errori
        if (movie == null)
            return false;
        ...
        // disabilito le proiezioni che hanno lo stesso id del film ma non reatroattivamente
...
## OperatoreService.cs V1.2

Andrea Bruno 25-05-2026
Cambiato da FindAsync a FirstOrDefaultAsync nel metodo OttieniBigliettoTramiteIdAsync per la ricerca del biglietto tramite id
```c#
using Microsoft.AspNetCore.Identity;
using Microsoft.EntityFrameworkCore;
using NuovoCinemaParadiso.Data;
...
public class OperatoreService
{
    ...
    public async Task<DtoBiglietto> OttieniBigliettoTramiteIdAsync(string id)
    {
        Biglietto? biglietto = await _contesto.Biglietti
            ...
            // Cerca nel database il primo Biglietto con Id uguale a quello passato.
            // A differenza di FindAsync, questa query funziona anche se la chiave primaria
            // dell'entità Biglietto NON è "Id" oppure se EF non riconosce "Id" come PK.
            // Se non viene trovato alcun biglietto, viene lanciata una NotFoundException.
            ?? throw new NotFoundException("Biglietto", id);
        Proiezione? proiezione = await _contesto.Proiezioni.FindAsync(biglietto.ProiezioneId)
            ?? throw new NotFoundException("Proiezione", biglietto.ProiezioneId);
        Movie? movie = await _contesto.Movies.FindAsync(proiezione.MovieId)
...
## OperatoreService.cs V1.3

Utente: Alessandro Gregorio
Data: 09/06/2026
Descrizione: Adesso viene correttamente inoltrato il saldo al controller nelle funzioni service OttieniUtentiAsync() e OttieniUtenteTramiteIdAsync()
...
public class OperatoreService
{
    ...
    public async Task<List<DtoUtente>> OttieniUtentiAsync()
    {
        ...
        for (int i = 0; i < utenti.Count; i++)
        {
            ...
            dto.DataInizioAbbonamento = utenteCorrente.DataInizioAbbonamento;
            dto.TipoAbbonamento = abbonamento?.Nome ?? string.Empty;
            //aggiunto passaggio del saldo all'interno del dto
...
## ProiezioneService.cs v 1.2

- Utente: Marco Strazzeri
- Data: 09/06/2026
- Descrizione:  aggiunta gestione degli errori nella funzione di modifica
...
public class ProiezioneService
{
    ...
    {
        Proiezione? proiezione = await _contesto.Proiezioni.FindAsync(id);
        //aggiunta la lettura di film sala e turno tramite gli id ricevuti
        Movie? film = await _contesto.Movies.FindAsync(dto.MovieId);
        Sala? sala = await _contesto.Sale.FindAsync(dto.SalaId);
        Turno? turno = await _contesto.Turni.FindAsync(dto.TurnoId);
        
        //gestione degli errori in caso di null
        if (proiezione == null)
        {
            return false;
        }
...
Aggiunto il parametro IsDeleted al passaggio tra il modello e i dto
...
public class SalaService
{
    ...
    public async Task<List<DtoSala>> OttieniTuttoAsync()
    {
        ...
        for (int i = 0; i < sale.Count; i++)
        {
            ...
            DtoSala dto = new DtoSala
            {
                ...
                NomeTipologia = tipologiaSala?.Nome ?? "",
                TipologiaSalaId = tipologiaSala?.Id ?? "",
                //aggiunto passaggio di parametri
    ...
    public async Task<bool> EliminaAsync(string id)
    {
        ...
        //Cambiato metodo di eliminazione
        sala.IsDeleted = true;
        //_contesto.Sale.Remove(sala);
        await _contesto.SaveChangesAsync();
...
Utente: Marco Strazzeri
Data: 09/06/2026
Dettagli: Corretto il messaggio di errore in caso di eliminazione di una sala già eliminata
   ...
   public async Task<string?> ModificaAsync(string id, DtoCreazioneSala dto)
{
    ...
    {
        Sala? sala = await _contesto.Sale.FindAsync(id);
        //aggiunto controllo se la sala è già stata eliminata tramite il booleano (disattivata)
...
## UtenteService.cs V1.2
Utente: Lorenzo Laviosa
Data: 27/05/2026
Descrizione: Aggiunta creazione nel metodo di ricarica, viene sistemato il ritorno nel metodo di riscatto
...
public class UtenteService
{
    ...
    // 1. Aggiungi utenteId come parametro (ricordati di passarlo dal Controller come ci siamo detti prima!)
    public async Task<(bool Successo, string Messaggio, DtoCreazioneGiftCard? Dto)>
        RiscattaGiftCardAsync(string utenteId, DtoCodiceRiscatto dto)
    {
        ...
        // 2. Recupera l'utente che sta riscattando la carta
        Utente? utenteCorrente = await _gestioneUtenti.FindByIdAsync(utenteId);
        if (utenteCorrente == null)
            return (false, "Utente non trovato.", null);

        // 3. AGGIORNAMENTI CRUCIALI
        trovata.Riscattata = true;
        trovata.UtenteId = utenteId; // Assegna la carta a chi l'ha riscattata (niente più NULL nel database!)
        utenteCorrente.Saldo += trovata.Valore; // VERSA I SOLDI SUL CONTO DELL'UTENTE!
...
### UtenteService.cs Versione 1.4

Utente: Marco Strazzeri
Data: 5/06/2026
Descrizione: in abbonati() aggiunta modifica conto cinema e controllo scadenza dell'abbonamento

<details><summary>Versione 1.4</summary>
...
public class UtenteService
{
    ...
    public async Task<(bool Successo, string Messaggio)> AbbonatiAsync(string abbonamentoId, string utenteId)
    {
        ...
        if (utenteTrovato.SeAbbonato)
        {
            //aggiunta controllo data scadenza
...
Utente: Marco Strazzeri
Data: 5/06/2026
Descrizione: in RicaricaGiftCard() aggiunta modifica conto cinema

<details><summary>Versione 1.5</summary>
...
### UtenteService.cs Versione 1.6

Utente: Alessandro Gregorio
09/06/2026
Details: Quando una giftcard viene creata all'utente li viene passato il codice di attivazione
...
public class UtenteService
{
    ...
    public async Task<(bool Successo, string Messaggio)> RicaricaGiftCardAsync(string utenteId, DtoRicaricaGiftCard dto)
    {
        ...
        utenteCorrente.Saldo -= dto.Importo;
        await _contesto.SaveChangesAsync();
        // Adesso il messaggio di avvenuto pagamento restiuisce il codice della giftcard

        return (true, $"Gift card creata correttamente: {nuovaGiftCard.CodiceRiscatto}");
    }

    public async Task<(bool Successo, string Messaggio, DtoCreazioneGiftCard? Dto)> RiscattaGiftCardAsync(string utenteId, DtoCodiceRiscatto dto)
    {
        List<GiftCard> tutte = _contesto.GiftCards.ToList();
        GiftCard? trovata = null;
