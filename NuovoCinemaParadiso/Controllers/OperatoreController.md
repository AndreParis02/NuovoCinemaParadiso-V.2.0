...
public class OperatoreController : ControllerBase
{
    ...
    public async Task<ActionResult<List<DtoUtente>>> OttieniUtentiTramiteAbbonamentoAsync(string abbonamentoId)
    {
        ...
        try
        {
            //ottiene gli utenti associati a un abbonamento specifico. Lancia eccezione se l'abbonamento non esiste
        ...
        catch (NotFoundException)   //ritorna un errore specifico se l'abbonamento non è stato trovato
        {
            await _logAzioniService.SalvataggioLogAzioneAsync(utenteId, "Ottieni gli utenti per abbonamento", false);
            return NotFound($"Abbonamento con id {abbonamentoId} non trovato");
        }
